...
class Resources implements iUseAuthentication, iProvideMultiVersionApi
{
    ...
    public function get($id = '')
    {
        ...
        foreach ($routes as $value) {
            foreach ($value as $httpMethod => $route) {
                ...
                $fullPath = $route['url'];
                if ($fullPath !== $target && !StringHelper::beginsWith($fullPath, $target)) {
                    continue;
                }
                $fLen = strlen($fullPath);
                if ($tSlash) {
                    if ($fLen != $tLen && !StringHelper::beginsWith($fullPath, $target . '/'))
                        continue;
                } elseif ($fLen > $tLen + 1 && $fullPath{$tLen + 1} != '{' && !StringHelper::beginsWith($fullPath, '{')) {
                ...
                foreach (static::$excludedPaths as $exclude) {
                    if (empty($exclude)) {
                        if ($fullPath == $exclude)
                            continue 2;
                    } elseif (StringHelper::beginsWith($fullPath, $exclude)) {
                        continue 2;
                    }
                }
                $m = $route['metadata'];
    ...
    protected function _mapResources(array $allRoutes, array &$map, $version = 1)
    {
        foreach ($allRoutes as $fullPath => $routes) {
            ...
            if ($resource == 'resources' || StringHelper::endsWith($resource, 'index'))
                continue;
