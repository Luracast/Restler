    ...
    private function setType(&$object, ValidationInfo $info)
    {
        ...
        if ($info->type == 'array') {
            if ($info->children) {
                ...
                /**
                 * Bugfix:
                 * The model will not be shown, if we configure the model (via @type-phpDoc-synthax) inside an array!
                 *
                 * Example (phpDoc/annotations in custom domain-model with namespace):
