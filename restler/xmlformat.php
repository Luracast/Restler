...
class XmlFormat implements IFormat {
    ...
    {
        return $this->toXML ( RestlerHelper::objectToArray 
                ( $data, false ), self::$rootName, $humanReadable );
    }
    ...
    public function isAssoc($array)
    {
        return (is_array ( $array ) && 0 !== 
                count ( array_diff_key ( $array, array_keys 
                        ( array_keys ( $array ) ) ) ));
    }
     ...
     */
    public function toXML($data, $rootNodeName = 'result', 
            $humanReadable = false, &$xml = null)
    {
    ...
    public function toArray($xml, $ns = null, $firstCall = true)
    {
        ...
        if ($firstCall) {
            ...
            if (self::$parseNamespaces) {
                foreach ($xml->getDocNamespaces 
                        ( true ) as $namespace => $uri) {
                    if ($namespace == '') {
