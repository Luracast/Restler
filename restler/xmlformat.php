...
class XmlFormat implements iFormat {
	...
	public function encode($data, $humanReadable = false) {
		return $this->toXML ( RestlerHelper::objectToArray ( $data, false ), self::$rootName, $humanReadable );
	}
	 ...
	 */
	public function toXML($data, $rootNodeName = 'result', $humanReadable = false, &$xml = null) {
		...
		if (is_array ( $data )) {
			...
			$xml = @simplexml_load_string ( "<$rootNodeName>" . htmlspecialchars ( $data ) . "</$rootNodeName>" );
			// $xml->{0} = $data;
		}
		if (! $humanReadable) {
			return $xml->asXML ();
