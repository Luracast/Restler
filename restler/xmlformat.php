...
class XmlFormat implements iFormat
{
    ...
    {
        return $this->toXML(object_to_array($data, false), self::$rootName, 
        $humanReadable);
    }
     ...
     */
    public function toXML ($data, $rootNodeName = 'result', $humanReadable = false, 
    &$xml = null)
    {
        ...
        if (is_array($data)) {
            ...
            foreach ($data as $key => $value) {
                ...
                //call this function
                if (is_array($value)) {
                    $node = $this->isAssoc($value) || $numeric ? $xml->addChild(
                    $key) : $xml;
                    // recrusive call.
                    if ($numeric)
                        $key = 'anon';
            ...
            "<$rootNodeName>" . htmlspecialchars($data) . "</$rootNodeName>");
             //$xml->{0} = $data;
        }
        if (! $humanReadable) {
            return $xml->asXML();
        ...
        if (self::$parseAttributes) {
            foreach ($ns as $namespace => $uri) {
                ...
                foreach ($xml->attributes($uri) as $attName => $attValue) {
                    echo "ATTRIB " . $attName . " of NAME " . $xml->getName() .
                     PHP_EOL;
