...
class Restler {
	...
	public function setApiClassPath($path) {
		$this->_apiClassPath = ! empty ( $path ) && $path {0} == '/' ? $path : $_SERVER ['DOCUMENT_ROOT'] . dirname ( $_SERVER ['SCRIPT_NAME'] ) . '/' . $path;
		$this->_apiClassPath = rtrim ( $this->_apiClassPath, '/' );
	}

	public function setAPIVersion($version, $minimum = 0, $apiClassPath = '') {
		if (! is_int ( $version ))
			throw new InvalidArgumentException ( 'version should be an integer' );
				...
				if ($o->methodFlag) {
					...
					foreach ( $this->_authClasses as $authClass ) {
						...
						if (! method_exists ( $authObj, $authMethod )) {
							throw new RestException ( 401, 'Authentication Class ' . 'should implement iAuthenticate' );
						} elseif (! $authObj->$authMethod ()) {
							throw new RestException ( 401 );
						}
					}
				}
				$this->applyClassMetadata ( get_class ( $this->requestFormat ), $this->requestFormat, $o );
	...
	public function sendData($data, $statusCode = 0, $statusMessage = NULL) {
		...
		$this->applyClassMetadata ( $this->responder, $responder, $this->_serviceMethodInfo );
		if ($statusCode == 0) {
			$data = $this->responseFormat->encode ( $data, ! $this->_productionMode );
			$data = $responder->__formatResponse ( $data );
			$postProcess = '_' . $this->_serviceMethod . '_' . $this->responseFormat->getExtension ();
			if (isset ( $this->_serviceClassInstance ) && method_exists ( $this->_serviceClassInstance, $postProcess )) {
			...
			$message = $this->_codes [$statusCode] . (empty ( $statusMessage ) ? '' : ': ' . $statusMessage);
			$this->setStatus ( $statusCode );
			$data = $this->responseFormat->encode ( $responder->__formatError ( $statusCode, $message ), ! $this->_productionMode );
		}
		die ( $data );
	}
	...
	public function setStatus($code) {
		if (isset ( $_GET ['suppress_response_codes'] ) && $_GET ['suppress_response_codes'] == 'true')
			$code = 200;
		header ( "{$_SERVER['SERVER_PROTOCOL']} $code " . $this->_codes [strval ( $code )] );
	}
	...
	public function saveCache() {
		...
		foreach ( $this->_routes as $key => $value ) {
			$s .= PHP_EOL . PHP_EOL . PHP_EOL . "//############### $key ###############" . PHP_EOL . PHP_EOL;
			$s .= '$o[\'' . $key . '\']=array();';
		...
		@chmod ( $file, 0777 );
		if ($r === FALSE)
			throw new Exception ( "The cache directory located at '$this->cacheDir' needs to have " . 'the permissions set to read/write/execute for everyone ' . 'in order to save cache and improve performance.' );
	}
	...
	protected function getPath() {
		...
		$path = urldecode ( $this->removeCommonPath ( $fullPath, $_SERVER ['SCRIPT_NAME'] ) );
	...
	protected function applyClassMetadata($className, $instance, $methodInfo) {
		if (isset ( $methodInfo->metadata [$className] ) && is_array ( $methodInfo->metadata [$className] )) {
	...
	protected function generateMap($className, $basePath = '') {
		...
		foreach ( $methods as $method ) {
			...
			$methodFlag = $method->isProtected () ? (isRestlerCompatibilityModeEnabled () ? 2 : 3) : (isset ( $metadata ['protected'] ) ? 1 : 0);
			...
			if (preg_match_all ( '/@url\s+(GET|POST|PUT|PATCH|DELETE|HEAD|OPTIONS)[ \t]*\/?(\S*)/s', $doc, $matches, PREG_SET_ORDER )) {
				...
				if ($methodUrl == 'index')
					$methodUrl = '';
				$url = empty ( $methodUrl ) ? rtrim ( $basePath, '/' ) : $basePath . $methodUrl;
				...
				$position = 1;
				foreach ( $params as $param ) {
					if (($param->isOptional () && ! $allowAmbiguity) || $param->getName () == 'request_data') {
						break;
					}
					if (! empty ( $url ))
						$url .= '/';
